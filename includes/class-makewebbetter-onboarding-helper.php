...
class Makewebbetter_Onboarding_Helper {
	...
	public function enqueue_styles() {
		...
		if ( $this->is_valid_page_screen() ) {
			...
			wp_enqueue_style( 'makewebbetter-onboarding-style', ONTRAWOO_URL . 'admin/css/makewebbetter-onboarding-admin.css', array(), '1.0.0', 'all' );

			// Uncomment Only when your plugin doesn't uses the Select2
			wp_enqueue_style( 'makewebbetter-onboarding-select2-style', ONTRAWOO_URL . 'admin/css/select2.min.css', array(), '1.0.0', 'all' );
		}
	}
	...
	public function enqueue_scripts() {
		...
		if ( $this->is_valid_page_screen() ) {
			...
			wp_enqueue_script( 'makewebbetter-onboarding-scripts', ONTRAWOO_URL . 'admin/js/makewebbetter-onboarding-admin.js', array( 'jquery' ), '1.0.0', true );
			...
			// Uncomment Only when your plugin doesn't uses the Select2
			wp_enqueue_script( 'makewebbetter-onboarding-select2-script', ONTRAWOO_URL . 'admin/js/select2.min.js', array( 'jquery' ), '1.0.0', false );
		}
	}
	...
	public function add_onboarding_popup_screen() {
		...
		if ( $this->is_valid_page_screen() && $this->can_show_onboarding_popup() ) {
			require_once ONTRAWOO_DIR_PATH . 'extra-templates/makewebbetter-onboarding-template-display.php';
		}
	}
	...
	public function add_deactivation_popup_screen() {
		...
		if ( ! empty( $pagenow ) && 'plugins.php' == $pagenow ) {
			require_once ONTRAWOO_DIR_PATH . 'extra-templates/makewebbetter-deactivation-template-display.php';
		}
	}
	...
	public function is_valid_page_screen() {
		...
		if ( ! empty( 'admin.php' == $pagenow ) && ( 'ontrawoo' == $_GET['page'] ) ) {
			$is_valid = true;
		}
	...
	public function can_show_onboarding_popup() {
		...
		$is_already_sent = get_option( 'ontrawoo_onboarding_data_sent', false );
		...
		$get_skipped_timstamp = get_option( 'ontrawoo_onboarding_skipped', false );
	...
	public function add_on_boarding_form_fields() {
		...
		$fields = array(
			...
			rand() => array(
				'id' => 'monthly-revenue',
				'label' => esc_html__( 'What is your monthly revenue?', ONTRAWOO_INTEGRATION ),
				'type' => 'radio',
			...
			rand() => array(
				'id' => 'industry_type',
				'label' => esc_html__( 'What industry defines your business?', ONTRAWOO_INTEGRATION ),
				'type' => 'select',
				'name' => 'industry_type_',
				'value' => '',
				'multiple' => 'yes',
			...
			rand() => array(
				...
				'type' => 'hidden',
				'name' => 'show-counter',
				'value' => get_option( 'ontrawoo_onboarding_data_sent', 'not-sent' ),
				'required' => '',
				'extra-class' => '',
			),
	...
	public function send_onboarding_data() {
		...
		if ( ! empty( $action_type ) && 'onboarding' == $action_type ) {
			$get_skipped_timstamp = update_option( 'ontrawoo_onboarding_data_sent', 'sent' );
		}
	...
	public function skip_onboarding_popup() {
		...
		$get_skipped_timstamp = update_option( 'ontrawoo_onboarding_skipped', time() );
